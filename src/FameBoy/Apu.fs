...
[<Literal>]
let ringBufferSize = 2 * bufferSize
...
module private SineWave =
    // TODO send to Raylibhttps://www.raylib.com/examples/audio/loader.html?name=audio_raw_stream
    let sineFrequency = 200f // hz

    let at apuTime : float32 =
        let t = float32 apuTime / (float32 (cpuFrequency * 4))

        sin (6.28318f * t * sineFrequency)
...
let stepApu (state: Apu) =
    ...
    if state.Accumulator > tCyclesPerSample then
        ...
        if state.WriteHead - state.ReadHead < ringBufferSize then
            ...
            state.RingBuffer[i] <- SineWave.at state.Timer
