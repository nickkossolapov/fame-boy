...
let ``Load register (immediate) - ld b,n`` () =
    ...
    cpu.Pc <- 0x100us
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0x42uy
...
let ``Load from immediate (indirect HL) - ld (hl),n`` () =
    ...
    cpu.Registers.HL <- 0xC000us
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0xABuy
...
let ``Load accumulator (direct) - ld a,(nn)`` () =
    ...
    cpu.Pc <- 0x100us
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0x50uy // LSB
    cpu.Memory.Cartridge.Rom[0x102] <- 0xC0uy // MSB
...
let ``Load accumulator (direct 0xFF00+n) - ldh a,(n)`` () =
    ...
    cpu.Pc <- 0x100us
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0x80uy
    cpu.Memory[0xFF80us] <- 0xDEuy
...
let ``Load 16-bit register - ld bc,nn`` () =
    ...
    cpu.Pc <- 0x100us
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0xFEuy // LSB
    cpu.Memory.Cartridge.Rom[0x102] <- 0xCAuy // MSB
...
let ``Load from stack pointer (direct) - ld (nn),sp`` () =
    ...
    cpu.Sp <- 0xABCDus
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0x50uy // LSB of nn
    cpu.Memory.Cartridge.Rom[0x102] <- 0xC0uy // MSB of nn
...
let ``Load HL from adjusted stack pointer - ld hl,sp+e`` () =
    ...
    cpu.Sp <- 0xFEFEus
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0x02uy // e = 2
...
let ``Load HL from adjusted stack pointer (negative) - ld hl,sp+e`` () =
    ...
    cpu.Sp <- 0xFF01us
    cpu.Memory.Cartridge.Rom[0x100] <- opcode
    cpu.Memory.Cartridge.Rom[0x101] <- 0xFEuy // e = -2
